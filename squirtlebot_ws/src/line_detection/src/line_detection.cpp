    ...
    ****************************************************************/
    void LineDetector::blobs_callback(const cmvision::Blobs blobs_in){
        ...
        if(n>0){
            ...
            } else current_line.intercept = image_h/2;
            ...
            //update filters
            update_filter(slope_filter, _LD_SLOPE_MEM, current_line.slope);
            update_filter(intercept_filter, _LD_INTERCEPT_MEM, current_line.intercept);
            update_filter(centroid_filter, _LD_CENTROID_MEM, current_line.centroid);    
        } else {
